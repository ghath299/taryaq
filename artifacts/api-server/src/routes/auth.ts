...
async function sendTelegramOtp(phone: string, otp: string): Promise<void> {
  const token = process.env["TELEGRAM_BOT_TOKEN"];
  const chatId = process.env["TELEGRAM_CHAT_ID"];
  if (!token || !chatId) {
    logger.info({ phone }, `[OTP] ${otp} → console (no Telegram config)`);
    return;
  }
  const text = `رمز تريتاق:\n*${otp}*\nللرقم: ${phone}\nصالح 5 دقائق`;
  ...
  if (!resp.ok) {
    ...
    logger.error({ status: resp.status, body }, "[OTP] Telegram send failed");
  }
}

/* ---------------------------- Routes ---------------------------- */

// إرسال OTP عبر Telegram (أو console في dev)
router.post("/send-otp", async (req: Request, res: Response) => {
  ...
  // Master test credentials — bypass everything
  if (
    phoneRaw === CONFIG.MASTER_PHONE
  ) {
  ...
  await logSecurity({ phoneHash, action: "otp_request", ip, meta: { channel: "telegram" } });

  try {
    await sendTelegramOtp(phoneRaw, otp);
  } catch (e) {
    logger.error({ e }, "[OTP] Telegram delivery error (non-fatal)");
  }

  res.json({ success: true, sentAt });
});

// التحقق من OTP المُرسَل عبر Telegram
router.post("/verify-otp", async (req: Request, res: Response) => {
  ...
  const phoneRaw =
    typeof body.phoneNumber === "string" ? sanitizePhone(body.phoneNumber) : "";
  const codeRaw =
    typeof body.code === "string" ? body.code.replace(/\D/g, "").slice(0, 6) : "";
  ...
  if (!phoneRaw || !codeRaw || codeRaw.length !== 6) {
    res.status(400).json({ success: false, message: "بيانات غير مكتملة" });
    return;
  }

  // Master credentials bypass
  if (phoneRaw === CONFIG.MASTER_PHONE && codeRaw === CONFIG.MASTER_OTP) {
  ...
  if (!record) {
    res.status(400).json({ success: false, message: "لا يوجد رمز — أعد الإرسال" });
    return;
  }
  if (Date.now() > record.expiry) {
    await deleteOtp(phoneRaw);
    res.status(400).json({ success: false, message: "انتهت صلاحية الرمز", expired: true });
    return;
  }

  if (sha256(codeRaw) !== record.otpHash) {
    const attemptsRemaining = CONFIG.OTP_MAX_VERIFY_ATTEMPTS - dec.verifyAttempts;
    await logSecurity({ phoneHash, action: "verify_failed", ip, meta: { reason: "wrong_otp" } });
    res.status(400).json({
      success: false,
  ...
  await deleteOtp(phoneRaw);
  await logSecurity({ phoneHash, action: "verify_success", ip, meta: { channel: "telegram" } });
...
// تسجيل البيانات + rate limiting قبل إرسال OTP من Firebase (legacy)
